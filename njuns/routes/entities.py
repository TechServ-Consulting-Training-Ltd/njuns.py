...
class EntitySearchCondition:
    ...
    def __init__(
        ...
        *,
        ...
        if (
            group is EntitySearchGroup
            and (conditions is None or len(conditions if conditions else [])) == 0
        ):
            raise ValueError("Child conditions must be specified when grouping")

        if (
            operator is EntitySearchOperator
            and not (
                operator == EntitySearchOperator.IN
                or operator == EntitySearchOperator.NOTIN
            )
            and value is not List
        ):
...
class EntitiesRoute(BaseRoute):
    ...
    async def create_entity(
        ...
        :param entity_name: Entity name.
        :type entity_name: str
        :param data: A JSON object with the entity.
